...
public:
  ...
  ~RegistryKey() {
    if ((m_hKey != nullptr) && !((m_hKey >= HKEY_CLASSES_ROOT) &&
#if (WINVER >= 0x0400)
  ...
  /// <summary>
  ///		Member method to open registry key on specified path, with specified
  ///access rights
  /// </summary>
  ...
  ///	</param>
  /// <exception></exception>
  RegistryKey_ptr Open(const StringType &path,
                       DesiredAccess access = DesiredAccess::Read) {
    if (path.empty()) {
      throw std::invalid_argument(
          "Specified path to registry key cannot be empty");
    }
    ...
    LSTATUS lStatus = RegOpenKeyEx(m_hKey, path.c_str(), dwOptions,
                                   static_cast<REGSAM>(access), &hKey);
  ...
  RegistryKey_ptr CreateVolatile(const StringType &path,
                                 DesiredAccess access = DesiredAccess::Read) {
  ...
  RegistryKey_ptr
  Create(const StringType &path, DesiredAccess access = DesiredAccess::Read,
         CreateKeyOptions options = CreateKeyOptions::NonVolatile) {
    if (path.empty()) {
      throw std::invalid_argument(
          "Specified path to registry key cannot be empty");
    }
    ...
    LSTATUS lStatus = RegCreateKeyEx(
        m_hKey, path.c_str(), dwReserved, lpClass, static_cast<DWORD>(options),
        static_cast<REGSAM>(access), lpSecurityAttributes, &hKey, nullptr);
  ...
  void Delete(const StringType &name) {
  ...
  void Save(const StringType &file) {
    LPSECURITY_ATTRIBUTES lpSecurityAttributes = nullptr;
  ...
  /// </summary>
  /// <param name="path">Subkey relative path to be checked for
  /// existence</param>
  bool HasKey(const StringType &path) {
    if (path.empty()) {
      throw std::invalid_argument(
          "Specified path to registry key cannot be empty");
    }
  ...
  bool Exists(const StringType &path) { return HasKey(path); }

  bool HasValue(const StringType &name) {
    if (name.empty()) {
    ...
    LSTATUS lStatus = RegQueryValueEx(m_hKey, name.c_str(), nullptr, nullptr,
                                      nullptr, nullptr);
    if (lStatus == ERROR_SUCCESS) {
      hasValue = true;
    } else if (lStatus != ERROR_FILE_NOT_FOUND) {
  ...
  bool GetBoolean(const StringType &name) {
    DWORD dwType = 0;
    DWORD dwData = 0;
    ...
    LSTATUS lStatus =
        RegQueryValueEx(m_hKey, name.c_str(), lpReserver, &dwType,
                        reinterpret_cast<LPBYTE>(&dwData), &cbData);
    if (lStatus != ERROR_SUCCESS) {
      auto ec = std::error_code(lStatus, std::system_category());
    ...
    if (dwType != REG_DWORD && dwType != REG_QWORD) {
      throw std::runtime_error("Wrong registry value type " +
                               std::to_string(dwType) + " for boolean value.");
    }
  ...
  bool GetBoolean() { return GetBoolean(TEXT("")); }

  void SetBoolean(const StringType &name, bool value) {
    DWORD dwValue = value ? 1 : 0;
    DWORD cbData = sizeof(dwValue);

    LSTATUS lStatus =
        RegSetValueEx(m_hKey, name.c_str(), 0, REG_DWORD,
                      reinterpret_cast<const LPBYTE>(&dwValue), cbData);
    if (lStatus != ERROR_SUCCESS) {
      auto ec = std::error_code(lStatus, std::system_category());
  ...
  void SetBoolean(bool value) { SetBoolean(TEXT(""), value); }

  long GetInt32(const StringType &name) {
    long lData = 0;
    DWORD cbData = sizeof(lData);

    LSTATUS lStatus =
        RegQueryValueEx(m_hKey, name.c_str(), nullptr, nullptr,
                        reinterpret_cast<LPBYTE>(&lData), &cbData);
    if (lStatus != ERROR_SUCCESS) {
      auto ec = std::error_code(lStatus, std::system_category());
  ...
  long GetInt32() { return GetInt32(TEXT("")); }

  unsigned long GetUInt32(const StringType &name) {
    return static_cast<unsigned long>(GetInt32(name));
  }

  unsigned long GetUInt32() { return GetUInt32(TEXT("")); }

  void SetInt32(const StringType &name, long value) {
    DWORD cbData = sizeof(value);

    LSTATUS lStatus =
        RegSetValueEx(m_hKey, name.c_str(), 0, REG_DWORD,
                      reinterpret_cast<const LPBYTE>(&value), cbData);
    if (lStatus != ERROR_SUCCESS) {
      auto ec = std::error_code(lStatus, std::system_category());
  ...
  void SetInt32(long value) { return SetInt32(TEXT(""), value); }

  void SetUInt32(const StringType &name, unsigned long value) {
    return SetInt32(name, static_cast<long>(value));
  }

  void SetUInt32(unsigned long value) { return SetUInt32(TEXT(""), value); }

  long long GetInt64(const StringType &name) {
    long long llData = 0;
    DWORD cbData = sizeof(llData);

    LSTATUS lStatus =
        RegQueryValueEx(m_hKey, name.c_str(), nullptr, nullptr,
                        reinterpret_cast<LPBYTE>(&llData), &cbData);
    if (lStatus != ERROR_SUCCESS) {
      auto ec = std::error_code(lStatus, std::system_category());
  ...
  long long GetInt64() { return GetInt64(TEXT("")); }

  unsigned long long GetUInt64(const StringType &name) {
    return static_cast<unsigned long long>(GetInt64(name));
  }

  unsigned long long GetUInt64() {
    return static_cast<unsigned long long>(GetUInt64(TEXT("")));
  }

  void SetInt64(const StringType &name, long long value) {
    DWORD cbData = sizeof(value);

    LSTATUS lStatus =
        RegSetValueEx(m_hKey, name.c_str(), 0, REG_QWORD,
                      reinterpret_cast<const LPBYTE>(&value), cbData);
    if (lStatus != ERROR_SUCCESS) {
      auto ec = std::error_code(lStatus, std::system_category());
  ...
  void SetInt64(long long value) { return SetInt64(TEXT(""), value); }

  void SetUInt64(const StringType &name, unsigned long long value) {
    SetInt64(name, static_cast<long long>(value));
  }

  void SetUInt64(unsigned long long value) { SetUInt64(TEXT(""), value); }
  ...
  StringType GetString(const StringType &name) {
    ...
    LSTATUS lStatus = RegGetValue(m_hKey, nullptr, name.c_str(), dwFlags,
                                  &dwType, nullptr, &cbData);
    if (lStatus != ERROR_SUCCESS) {
      auto ec = std::error_code(lStatus, std::system_category());
    ...
    {
      throw std::runtime_error("Wrong registry value type " +
                               std::to_string(dwType) + " for string value.");
    }
    ...
    if (cbData) {
      ...
      lStatus = RegGetValue(m_hKey, nullptr, name.c_str(), dwFlags, &dwType,
      ...
      if (lStatus == ERROR_SUCCESS) {
        ...
        value = StringType(data);
      } else {
        auto ec = std::error_code(lStatus, std::system_category());

        pex = std::make_exception_ptr(
            std::system_error(ec, "RegGetValue() failed"));
      }
  ...
  StringType GetString() { return GetString(TEXT("")); }

  void SetString(const StringType &name, const StringType &value) {
    auto cbData = static_cast<DWORD>(value.length());

    LSTATUS lStatus = RegSetValueEx(
        m_hKey, name.c_str(), 0, REG_SZ,
        reinterpret_cast<const BYTE *>(value.c_str()), cbData * sizeof(TCHAR));
    if (lStatus != ERROR_SUCCESS) {
      auto ec = std::error_code(lStatus, std::system_category());
  ...
  void SetString(const StringType &value) { SetString(TEXT(""), value); }

  /// <summary>
  ///	Create registry value with specified name of type REG_EXPAND_SZ within
  ///this registry key
  /// </summary>
  /// <param name="name">Name of registry value (Empty string for default key
  /// value)</param> <param name="value">Value to be set</param>
  void SetExpandString(const StringType &name, const StringType &value) {
    auto cbData = static_cast<DWORD>(value.length());

    LSTATUS lStatus = RegSetValueEx(
        m_hKey, name.c_str(), 0, REG_EXPAND_SZ,
        reinterpret_cast<const BYTE *>(value.c_str()), cbData * sizeof(TCHAR));
    if (lStatus != ERROR_SUCCESS) {
      auto ec = std::error_code(lStatus, std::system_category());
  ...
  /// <summary>
  ///	Create default registry value of type REG_EXPAND_SZ within this registry
  ///key
  /// </summary>
  /// <param name="value">Value to be set</param>
  void SetExpandString(const StringType &value) {
    SetExpandString(TEXT(""), value);
  }
  ...
  void EnumerateSubKeys(const __Function &callback) {
    ...
    auto pszName = reinterpret_cast<TCHAR *>(
        LocalAlloc(LMEM_FIXED, dwLongestSubKeyLen * sizeof(TCHAR)));
    assert(pszName != nullptr);

    for (DWORD i = 0; i < dwSubKeys; ++i) {
      DWORD dwLen = dwLongestSubKeyLen;

      lStatus = RegEnumKeyEx(m_hKey,  // Key handle
                             i,       // Subkey index
      ...
      StringType subKeyName(pszName, dwLen);
  ...
  ///	If this parameter is true, the function reports changes in the specified
  ///key and its subkeys. 	Otherwise, the function reports changes only in the
  ///specified key.
  /// Default is false.
  /// </param>
  ...
  /// </param>
  void Notify(bool watchSubtree = false,
              NotifyFilter notifyFilter = NotifyFilter::ChangeName |
                                          NotifyFilter::ChangeAttributes) {
    HANDLE hEvent = nullptr;
    BOOL fAsynchronous = FALSE;

    LSTATUS lStatus = RegNotifyChangeKeyValue(
        m_hKey, watchSubtree ? TRUE : FALSE, static_cast<DWORD>(notifyFilter),
  ...
  /// <summary>
  ///	Notifies the caller about changes to the attributes or contents of a
  ///specified registry key asynchonously via provided event object.
  /// </summary>
  /// <param name="hEvent">
  ///	A handle to an event. Call to this function returns immediately and
  ///changes are reported by signaling this event.
  /// </param>
  /// <param name="watchSubtree">
  ///	If this parameter is true, the function reports changes in the specified
  ///key and its subkeys. 	Otherwise, the function reports changes only in the
  ...
  /// Default is ChangeName and ChangeAttributes.
  /// </param>
  void NotifyAsync(HANDLE hEvent, bool watchSubtree = false,
                   NotifyFilter notifyFilter = NotifyFilter::ChangeName |
                                               NotifyFilter::ChangeAttributes |
                                               NotifyFilter::ChangeLastSet) {
    if (hEvent == nullptr) {
